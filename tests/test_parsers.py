...
def test_make_contractions():
    ...
    with pytest.raises(TypeError):
        make_contractions(basis_dict, {"H", "C"}, np.array([[0, 0, 0], [1, 1, 1]]), "spherical")
    with pytest.raises(TypeError):
    ...
    with pytest.raises(TypeError):
        make_contractions(
            basis_dict, ["H", "H"], np.array([[0, 0, 0, 2], [1, 1, 1, 2]]), "spherical"
        )

    with pytest.raises(ValueError):
        make_contractions(
            basis_dict, ["H", "H", "H"], np.array([[0, 0, 0], [1, 1, 1]]), "spherical"
        )
