...
\section{Gaussian Primitive (Primitive)}
...
momentum, $\ell = a_x + a_y + a_z$, and the
\begin{equation}
  \label{eq:cart_norm}
  \begin{split}
    N (\alpha_i, \mathbf{a}) &=
...
Therefore, we will denote Cartesian primitives as primitives.

\subsection{Transformation of Spherical to Cartesian}
...
Generalized contractions is a set of contractions that have the same center
...
In \verb|gbasis|, contractions can be derivatized to orbitrary orders.
